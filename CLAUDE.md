...
9. **Async:** All port traits use `async_trait` (`?Send` on WASM). Core has no tokio dependency — uses `std::sync::RwLock` and `web_time::Instant`. Only `bolt402-lnd[grpc]` requires tokio.
